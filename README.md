...
|---|---|---|---|---|---|---|
| TC-A01 | Crear perfil atleta válido | POST /api/v1/atletas/ | `{"nombres": "Juan", "apellidos": "Perez", "fecha_nacimiento": "2000-05-15", "especialidad": "NATACION", "anios_experiencia": 5, "categoria": "SENIOR"}` | `{"success": true, "message": "Perfil creado exitosamente", "data": {"id": 1, "user_id": 10, "especialidad": "NATACION", "anios_experiencia": 5}, "errors": null}` (Status 201) | Usuario ATLETA autenticado, sin perfil previo | Exitoso |
| TC-A02 | Validar Usuario No Atleta | POST /api/v1/atletas/ | `{"nombres": "Juan", ...}` (Usuario ENTRENADOR) | `{"success": false, "message": "Solo usuarios con rol ATLETA pueden crear perfil deportivo", "errors": null}` (Status 403) | Usuario autenticado con rol ENTRENADOR | Fallido |
| TC-A03 | Validar Perfil Duplicado | POST /api/v1/atletas/ | `{"nombres": "Juan", ...}` (Usuario con perfil existente) | `{"success": false, "message": "El atleta ya existe", "errors": null}` (Status 400) | Usuario ATLETA con perfil existente | Fallido |
| TC-A04 | Usuario No Encontrado | POST /api/v1/atletas/ | `{"nombres": "Juan", ...}` (user_id inválido) | `{"success": false, "message": "Usuario no encontrado", "errors": null}` (Status 404) | user_id que no existe | Fallido |
| TC-A05 | Validar Especialidad | POST /api/v1/atletas/ | `{"especialidad": "ESPECIALIDAD_INVALIDA", ...}` | `{"success": false, "message": "Especialidad inválida", "errors": [{"field": "especialidad", "message": "..."}]}` | | Fallido |
| TC-A06 | Validar Años Experiencia | POST /api/v1/atletas/ | `{"anios_experiencia": -1, ...}` | `{"success": false, "message": "Validation Error", "errors": [{"field": "anios_experiencia", "message": "Debe ser >= 0"}]}` | | Fallido |
| TC-A07 | Crear con datos opcionales | POST /api/v1/atletas/ | `{"nombres": "Juan", "apellidos": "Perez", ...}` | `{"success": true, "message": "Perfil creado exitosamente", ...}` | Datos opcionales omitidos | Exitoso |
...
|---|---|---|---|---|---|---|
| TC-A08 | Ver mi perfil | GET /api/v1/atletas/me | Header: Authorization Bearer token | `{"success": true, "data": {"id": 1, "user_id": 10, "especialidad": "NATACION", "anios_experiencia": 5, ...}, "errors": null}` (Status 200) | Usuario autenticado con perfil de atleta | Exitoso |
| TC-A09 | Ver perfil de otro atleta | GET /api/v1/atletas/{atleta_id} | atleta_id: 5 | `{"success": true, "data": {"id": 5, "user_id": 12, "especialidad": "ATLETISMO", ...}, "errors": null}` (Status 200) | Usuario autenticado | Exitoso |
...
| TC-A11 | Listar todos los atletas | GET /api/v1/atletas/?skip=0&limit=100 | skip: 0, limit: 100 | `{"success": true, "data": [{"id": 1, ...}, {"id": 2, ...}, ...], "errors": null}` (Status 200) | | Exitoso |
| TC-A12 | Paginación | GET /api/v1/atletas/?skip=10&limit=10 | skip: 10, limit: 10 | Retorna atletas del 11 al 20 | | Exitoso |
...
| ID | Funcionalidad | Descripción | Datos de Entrada (JSON) | Salida Esperada (JSON) | Condiciones Previas | Resultado (Exitoso/Fallido) |
|---|---|---|---|---|---|---|
| TC-A14 | Actualizar datos básicos | PUT /api/v1/atletas/{atleta_id} | `{"especialidad": "ATLETISMO", "anios_experiencia": 10}` | `{"success": true, "data": {"id": 1, "especialidad": "ATLETISMO", "anios_experiencia": 10, ...}, "errors": null}` (Status 200) | Atleta existe | Exitoso |
| TC-A15 | Actualizar categoría | PUT /api/v1/atletas/{atleta_id} | `{"categoria": "MASTER"}` | `{"success": true, "data": {..., "categoria": "MASTER"}, ...}` (Status 200) | | Exitoso |
| TC-A16 | Actualizar parcial | PUT /api/v1/atletas/{atleta_id} | `{"especialidad": "NATACION"}` | Actualiza solo especialidad, mantiene otros campos | | Exitoso |
| TC-A17 | Atleta no encontrado | PUT /api/v1/atletas/9999 | `{"especialidad": "ATLETISMO", ...}` | `{"success": false, "message": "Atleta no encontrado", "errors": null}` (Status 404) | ID no existe | Fallido |
| TC-A18 | Validar especialidad actualizada | PUT /api/v1/atletas/{atleta_id} | `{"especialidad": "INVALIDA"}` | `{"success": false, "message": "Validation Error", ...}` (Status 422) | | Fallido |
...
| ID | Funcionalidad | Descripción | Datos de Entrada (JSON) | Salida Esperada (JSON) | Condiciones Previas | Resultado (Exitoso/Fallido) |
|---|---|---|---|---|---|---|
| TC-A21 | Crear historial médico | POST /api/v1/historiales-medicos/ | `{"talla": 1.80, "peso": 75, "alergias": "Ninguna", "enfermedades_hereditarias": "Ninguna", "enfermedades": "Ninguna"}` | `{"success": true, "message": "Historial creado", "data": {"id": 1, "imc": 23.15, "talla": 1.80, "peso": 75, ...}, "errors": null}` (Status 201) | Usuario ATLETA autenticado, sin historial previo | Exitoso |
| TC-A22 | Validar IMC calculado | POST /api/v1/historiales-medicos/ | `{"talla": 1.80, "peso": 75, ...}` | IMC retornado: 23.15 (75/(1.80^2)) | | Exitoso |
| TC-A23 | Ver historial médico | GET /api/v1/historiales-medicos/me | Header: Authorization Bearer token | `{"success": true, "data": {"id": 1, "imc": 23.15, "talla": 1.80, "peso": 75, ...}, "errors": null}` (Status 200) | Usuario autenticado con historial médico | Exitoso |
| TC-A24 | Actualizar historial | PUT /api/v1/historiales-medicos/{external_id} | `{"peso": 80}` | `{"success": true, "data": {"id": 1, "imc": 24.69, "peso": 80, ...}, "errors": null}` (Status 200) | Historial médico existe | Exitoso |
| TC-A25 | Validar dato duplicado | POST /api/v1/historiales-medicos/ | `{"talla": 1.80, "peso": 75, ...}` | `{"success": false, "message": "El usuario ya tiene historial médico", "errors": null}` (Status 400) | Historial médico ya existe | Fallido |
| TC-A26 | Usuario no es ATLETA | POST /api/v1/historiales-medicos/ | `{"talla": 1.80, "peso": 75, ...}` (Usuario ENTRENADOR) | `{"success": false, "message": "El usuario no existe o no es ATLETA", "errors": null}` (Status 400) | Usuario autenticado sin rol ATLETA | Fallido |
...
| TC-A28 | Listar historiales | GET /api/v1/historiales-medicos/?skip=0&limit=100 | skip: 0, limit: 100 | `{"success": true, "data": [{"id": 1, ...}, {"id": 2, ...}], "errors": null}` (Status 200) | | Exitoso |
| TC-A29 | Ver historial por ID | GET /api/v1/historiales-medicos/{external_id} | external_id válido | `{"success": true, "data": {...}, "errors": null}` (Status 200) | Historial existe | Exitoso |
...
| ID | Funcionalidad | Descripción | Datos de Entrada | Salida Esperada (JSON) | Condiciones Previas | Resultado (Exitoso/Fallido) |
|---|---|---|---|---|---|---|
| TC-A30 | Ver historial de competencias | GET /api/v1/atletas/historial | Header: Authorization Bearer token | `{"success": true, "data": [{"competencia_id": 1, "fecha": "2025-01-15", "resultado": {...}, ...}], "errors": null}` (Status 200) | Usuario autenticado con competencias | Exitoso |
